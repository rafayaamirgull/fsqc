    ...
    lines.extend(
        (
            ...
            _html_review_workflow(
                ...
                + re.sub(r"[^a-z0-9]+", "-", str(model["subject"]).lower()).strip("-"),
                analyzer=analyzer,
                model=model,
                visuals=visuals,
...
def format_markdown_document(
    ...
    lines = [
        "<!-- Canonical report generated by fsqc_report.py. fsqc-report.html is converted from this file. -->",
    ...
    for model, _, _, _ in report_items:
        ...
        icon = _disposition_icon(model["disposition"])
        subject_label = _md_escape(model["subject"])
        subject_anchor = _markdown_slug(f"Subject {model['subject']}")
        lines.append(
            f"| [`{subject_label}`](#{subject_anchor}) | {icon} {_md_escape(model['disposition'])} | "
    ...
    lines.extend(
        (
            ...
            "---",
            "",
            "<sub>Generated by <code>fsqc_report.py</code> from the FSQC metrics. "
            "The HTML report, when requested, is converted from this README. Research use only.</sub>",
        )
    )
    return "\n".join(lines) + "\n"
...
def _markdown_slug(value: str) -> str:
    """Return a stable HTML fragment identifier for a Markdown heading."""

    plain = re.sub(r"[`*_\[\]]", "", html.unescape(value)).lower()
    return re.sub(r"[^a-z0-9]+", "-", plain).strip("-") or "section"


def _markdown_table_cells(line: str) -> list[str]:
    """Split a generated pipe table row while preserving escaped pipes."""
...
def _is_markdown_table_separator(line: str) -> bool:
    """Return whether a line is a Markdown table alignment separator."""
...
def _markdown_inline(value: str) -> str:
    """Render the controlled inline Markdown emitted by this report generator."""

    tokens: dict[str, str] = {}

    def store(markup: str) -> str:
        key = f"@@FSQCINLINE{len(tokens)}@@"
        tokens[key] = markup
        return key

    def code_replacement(match: re.Match[str]) -> str:
        return store(f"<code>{html.escape(html.unescape(match.group(1)))}</code>")

    def image_replacement(match: re.Match[str]) -> str:
        alt = html.escape(html.unescape(match.group(1)), quote=True)
        source = html.escape(html.unescape(match.group(2)), quote=True)
        return store(
            f'<a class="inline-image-link" href="{source}"><img src="{source}" '
            f'alt="{alt}" loading="lazy"></a>'
        )

    def link_replacement(match: re.Match[str]) -> str:
        label = html.escape(html.unescape(match.group(1)))
        target = html.escape(html.unescape(match.group(2)), quote=True)
        return store(f'<a href="{target}">{label}</a>')

    value = re.sub(r"`([^`]+)`", code_replacement, value)
    value = re.sub(r"!\[([^\]]*)\]\(([^)]+)\)", image_replacement, value)
    value = re.sub(r"\[([^\]]+)\]\(([^)]+)\)", link_replacement, value)
    rendered = html.escape(html.unescape(value), quote=False)
    rendered = re.sub(r"\*\*([^*]+)\*\*", r"<strong>\1</strong>", rendered)
    rendered = re.sub(r"(?<!\*)\*([^*]+)\*(?!\*)", r"<em>\1</em>", rendered)
    # Compound constructs (for example a code span used as link text) create
    # outer placeholders after inner ones. Resolve in reverse creation order.
    for key, markup in reversed(tuple(tokens.items())):
    ...
    for label, css_class in (
        ...
        rendered = rendered.replace(
            label,
            f'<strong class="severity-badge severity-{css_class}">{label}</strong>',
        )
    return rendered


def _render_markdown_body(markdown_text: str) -> str:
    """Convert the generated README Markdown subset into semantic HTML."""
    ...
    def structural(line: str, next_line: str = "") -> bool:
        ...
        return (
            ...
            or (stripped.startswith("|") and _is_markdown_table_separator(next_line))
        )
    ...
    while index < len(lines):
        ...
        if heading:
            ...
            if level == 1:
                ...
                close_section()
                kind = "subject-section" if title.lower().startswith("subject ") else "document-section"
                rendered.append(
                    f'<section class="md-section {kind}" id="{anchor}">'
                    f'<h2>{_markdown_inline(title)}</h2>'
                )
                section_open = True
            else:
                rendered.append(
                    f'<h{level} id="{anchor}">{_markdown_inline(title)}</h{level}>'
                )
            index += 1
            continue
        if stripped in {"---", "___", "***"}:
        ...
        if stripped.startswith(">"):
            ...
            if quote_lines and re.fullmatch(r"\[![A-Z]+\]", quote_lines[0]):
                alert = quote_lines.pop(0)[2:-1].lower()
            content = " ".join(part for part in quote_lines if part)
            if alert:
                rendered.append(
                    f'<aside class="callout callout-{alert}"><span class="callout-label">'
                    f'{html.escape(alert.upper())}</span><p>{_markdown_inline(content)}</p></aside>'
                )
            else:
                rendered.append(f"<blockquote><p>{_markdown_inline(content)}</p></blockquote>")
            continue
        if stripped.startswith("|") and _is_markdown_table_separator(next_line):
            ...
            rendered.extend(f"<th>{_markdown_inline(cell)}</th>" for cell in headers)
            rendered.append("</tr></thead><tbody>")
            for row in rows:
                rendered.append("<tr>")
                rendered.extend(f"<td>{_markdown_inline(cell)}</td>" for cell in row)
                rendered.append("</tr>")
            ...
            while index < len(lines):
                ...
                if checkbox:
                    ...
                    item_html = (
                        ...
                        f'{_markdown_inline(checkbox.group(2))}</label>'
                    )
                else:
                    item_html = _markdown_inline(item)
        ...
        while index < len(lines):
            ...
            if structural(lines[index], following):
                break
            paragraph.append(lines[index].strip())
            index += 1
        rendered.append(f"<p>{_markdown_inline(' '.join(paragraph))}</p>")
...
MARKDOWN_REPORT_CSS = """
:root{--ink:#14233b;--muted:#5c6d82;--line:#d8e2ee;--paper:#fff;--wash:#f3f7fb;
--navy:#12345f;--blue:#1769aa;--cyan:#049eb3;--green:#18815a;--amber:#b77900;--red:#b42318}
...
.page-header{background:linear-gradient(130deg,#102846 0%,#1d4f80 58%,#137f95 130%);color:#fff;
padding:3rem max(1.25rem,calc((100% - 1180px)/2)) 2.7rem}.page-header-inner{max-width:940px}
.eyebrow{margin:0 0 .45rem;text-transform:uppercase;letter-spacing:.14em;font-size:.72rem;font-weight:800;
opacity:.8}.page-header h1{margin:0;font-size:clamp(2.2rem,5vw,3.7rem);line-height:1.04}.lede{max-width:820px;
margin:1rem 0 0;color:#dfeafa;font-size:1.08rem}.source-bar{position:sticky;z-index:20;top:0;display:flex;
justify-content:flex-end;padding:.55rem max(1.25rem,calc((100% - 1180px)/2));background:#102846ed;
backdrop-filter:blur(7px)}.source-bar a{color:#fff;text-decoration:none;font-size:.78rem;font-weight:750}
.markdown-body{max-width:1180px;margin:auto;padding:1.4rem 1.25rem 4rem}.md-section{margin:1.35rem 0;padding:1.45rem;
background:var(--paper);border:1px solid var(--line);border-radius:15px;box-shadow:0 9px 28px #17345c0d}
.md-section>h2{margin:-1.45rem -1.45rem 1.25rem;padding:1rem 1.45rem;border-radius:14px 14px 0 0;
background:linear-gradient(100deg,#eaf2fb,#eef9f8);color:var(--navy);font-size:1.55rem}
.subject-section>h2{background:linear-gradient(100deg,#dfeeff,#e6f8f6);font-size:1.8rem}
h3{margin:2rem 0 .8rem;color:var(--navy);font-size:1.25rem;border-bottom:1px solid #e3eaf2;padding-bottom:.45rem}
h4{margin:1.25rem 0 .55rem;color:#234e7d}a{color:#1d63a5}code{padding:.12rem .35rem;border-radius:4px;
background:#edf2f7;color:#163a63;font:90% ui-monospace,SFMono-Regular,Consolas,monospace}
pre{overflow:auto;padding:1rem;border-radius:8px;background:#122036;color:#e9f1fb}pre code{padding:0;background:none;color:inherit}
.callout{position:relative;margin:1.15rem 0;padding:.9rem 1rem .9rem 1.15rem;border:1px solid #b9d5ef;
border-left:5px solid var(--blue);border-radius:9px;background:#edf6ff}.callout p{margin:.25rem 0 0}.callout-label{
font-size:.7rem;font-weight:900;letter-spacing:.09em;color:var(--blue)}.callout-warning,.callout-caution{
border-color:#efd08b;border-left-color:#dc9a1d;background:#fff8e6}.callout-warning .callout-label,
.callout-caution .callout-label{color:#9a6500}.callout-important{border-left-color:#3268a8}.callout-note{
border-color:#ead695;border-left-color:#c89612;background:#fffbea}.callout-note .callout-label{color:#8d6500}
blockquote{margin:1rem 0;padding:.75rem 1rem;border-left:4px solid #82a9cf;background:#f5f8fc;color:#40546d}
blockquote p{margin:0}.table-scroll{overflow-x:auto;margin:.8rem 0 1.2rem}table{width:100%;border-collapse:separate;
border-spacing:0;margin:.8rem 0 1.2rem;border:1px solid var(--line);border-radius:9px;overflow:hidden}
th,td{padding:.68rem .75rem;border-right:1px solid var(--line);border-bottom:1px solid var(--line);vertical-align:top;
text-align:left}th:last-child,td:last-child{border-right:0}tr:last-child td{border-bottom:0}th{background:#e9f1fa;
color:#173e69;font-size:.78rem;text-transform:uppercase;letter-spacing:.025em}tbody tr:nth-child(even){background:#f8fafc}
ul,ol{padding-left:1.4rem}li{margin:.35rem 0}.check-item{display:inline-flex;align-items:flex-start;gap:.4rem}
details{margin:1rem 0;padding:.75rem 1rem;border:1px solid var(--line);border-radius:9px;background:#fbfdff}
summary{cursor:pointer;color:var(--navy);font-weight:750}details[open] summary{margin-bottom:.75rem}
img{max-width:100%;height:auto;border-radius:6px}td>a>img{display:block;margin:auto;background:#101722}
.severity-badge{display:inline-block;padding:.16rem .48rem;border-radius:999px;font-size:.76rem;white-space:nowrap}
.severity-high{color:#9d2018;background:#feeceb;border:1px solid #f4b8b2}.severity-review{color:#075da7;
background:#e8f3ff;border:1px solid #a9cff2}.severity-note{color:#855c00;background:#fff5c7;border:1px solid #e9cf68}
.severity-info{color:#126847;background:#e4f5ed;border:1px solid #addbc7}
.workflow-panel{position:relative;margin:1.4rem 0 2rem;padding:1.25rem;border:1px solid #c8d9eb;border-radius:14px;
background:linear-gradient(145deg,#f7faff,#f4f9ff 52%,#f4fbf8);overflow:hidden}.workflow-panel:before{content:"";
position:absolute;inset:0 0 auto;height:5px;background:linear-gradient(90deg,#2270c8,#00a2ba 35%,#315fb0 67%,#12987a)}
.workflow-heading{display:flex;justify-content:space-between;gap:1rem;margin:.3rem 0 1.1rem}.workflow-heading h3{
margin:.1rem 0!important;padding:0;border:0;font-size:1.55rem}.workflow-heading p{margin:.2rem 0;color:#41628f}
.status-legend{display:flex;flex-wrap:wrap;align-self:flex-start;gap:.55rem;padding:.5rem .65rem;border:1px solid #d6e2ef;
border-radius:8px;background:#fff}.status-legend span{display:flex;align-items:center;gap:.3rem;font-size:.7rem;white-space:nowrap}
.state-dot,.rail-state{display:inline-block;flex:0 0 auto;width:11px;height:11px;border-radius:50%;background:#8190a5}
.state-pass{background:#2da477}.state-review{background:#df951d}.state-correct{background:#df2941}.state-unknown,
.state-pending{background:#8190a5}.workflow-layout{display:grid;grid-template-columns:145px minmax(0,1fr);gap:1rem}
.workflow-rail{border-right:1px solid #cfdeef;padding-right:.8rem}.workflow-rail ol{list-style:none;margin:0;padding:0}
.workflow-rail li{display:grid;grid-template-columns:38px 1fr 9px;align-items:center;gap:.4rem;min-height:73px;
color:#496486;font-size:.67rem;font-weight:800;text-transform:uppercase}.rail-number{display:grid;place-items:center;width:38px;
height:38px;border:1px solid #abc5e4;border-radius:50%;background:#edf4fc;color:#346495;font-size:.9rem}
.workflow-board{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1rem}.flow-card{--stage-a:#276fc5;
--stage-b:#318bda;min-width:0;border:1px solid #bed8f1;border-radius:10px;background:#fff;overflow:hidden;
box-shadow:0 5px 14px #17345c10}.flow-card>header{display:grid;grid-template-columns:38px minmax(0,1fr);gap:.5rem;
align-items:center;padding:.7rem;color:#fff;background:linear-gradient(120deg,var(--stage-a),var(--stage-b))}
.flow-card>header h4{margin:0;color:#fff;font-size:.86rem}.card-number{display:grid;place-items:center;width:36px;height:36px;
border:1px solid #ffffff70;border-radius:50%;background:#ffffff24;font-weight:850}.stage-status{grid-column:1/-1;
justify-self:start;padding:.12rem .43rem;border-radius:999px;background:#fff;color:#4b5b70;font-size:.61rem;font-weight:850;
text-transform:uppercase}.status-pass{color:#187451}.status-review{color:#955b00}.status-correct{color:#a41f2f}
...
.stage-6{--stage-a:#008b83;--stage-b:#19a779}.flow-card-body{display:flex;flex-direction:column;gap:.6rem;padding:.72rem}
.card-prompt,.decision-question{margin:0;padding:.5rem;border:1px solid #d6e5f5;border-radius:7px;background:#f4f9fe;
color:#214a79;font-size:.78rem;font-weight:750;text-align:center}.evidence-ribbon,.reference-context{display:flex;
flex-direction:column;padding:.5rem .6rem;border-radius:7px;background:#edf5fe;color:#255482;font-size:.73rem}
.evidence-ribbon strong,.reference-context strong{font-size:.61rem;text-transform:uppercase;letter-spacing:.06em}
.stage-actions{display:flex;flex-direction:column;gap:.4rem;list-style:none;margin:0;padding:0}.stage-actions li{display:flex;
gap:.45rem;margin:0;padding:.48rem;border:1px solid #e1e9f2;border-radius:7px;background:#fbfdff}.stage-actions .state-dot{
margin-top:.2rem}.stage-actions strong{display:block;font-size:.73rem}.stage-actions p{margin:.1rem 0 0;color:#465b75;
font-size:.68rem}.workflow-thumbs{display:grid;grid-template-columns:repeat(3,1fr);gap:.3rem;margin-top:auto}
.workflow-thumb{display:block;min-width:0;border:1px solid #dce6f0;border-radius:6px;overflow:hidden;text-decoration:none}
.workflow-thumb img{display:block;width:100%;height:62px;object-fit:cover;border-radius:0;background:#101722}.workflow-thumb span{
display:block;padding:.22rem;background:#fff;color:#334f70;font-size:.55rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.decision-question{border:0;border-radius:999px;background:linear-gradient(100deg,#216bc0,#168ec2);color:#fff}
.decision-branches{display:grid;grid-template-columns:repeat(3,1fr);gap:.35rem}.decision-branches>div{display:flex;
flex-direction:column;padding:.5rem .3rem;border-radius:7px;text-align:center}.decision-branches strong{font-size:.69rem}
.decision-branches span{font-size:.59rem;font-weight:700}.decision-branches small{font-size:.52rem}.decision-accept{
background:#e3f5eb;color:#176b49}.decision-reprocess{background:#fff1cc;color:#825300}.decision-exclude{
background:#fee4e7;color:#9e1e31}.record-decision{padding:.48rem;border:1px solid #cddff1;border-radius:7px;
background:#edf5fd;color:#285586;font-size:.66rem;text-align:center}.workflow-outcome{margin:1rem 0 0 161px;padding:.65rem .85rem;
border-radius:8px;background:linear-gradient(90deg,#e8f1fc,#e9f8f3);color:#273d5c}
.footer{padding:2rem;text-align:center;color:var(--muted)}
@media(max-width:950px){.workflow-layout{grid-template-columns:1fr}.workflow-rail{display:none}.workflow-board{
grid-template-columns:repeat(2,minmax(0,1fr))}.workflow-outcome{margin-left:0}}
@media(max-width:650px){.page-header{padding-top:2.2rem}.markdown-body{padding-inline:.75rem}.md-section{padding:1rem}
.md-section>h2{margin:-1rem -1rem 1rem;padding:.85rem 1rem}.workflow-heading{flex-direction:column}.workflow-board{
grid-template-columns:1fr}.status-legend{align-self:stretch}.source-bar{position:static}.decision-branches{grid-template-columns:1fr}}
@media print{body{background:#fff}.source-bar{display:none}.page-header{background:#fff;color:#000;padding:1rem 0}
.lede{color:#333}.markdown-body{max-width:none;padding:0}.md-section,.workflow-panel,.flow-card{box-shadow:none;break-inside:avoid}
.workflow-board{grid-template-columns:1fr}.workflow-rail{display:none}.workflow-outcome{margin-left:0}}
"""


def format_html_from_markdown(
    markdown_text: str,
    markdown_path: Path,
    html_path: Path,
) -> str:
    """Convert the saved canonical README report into a styled HTML document."""

    title_match = re.search(r"^#\s+(.+)$", markdown_text, flags=re.MULTILINE)
    title = title_match.group(1).strip() if title_match else "Neuroimaging Quality-Control Report"
    readme_link = html.escape(_md_path(markdown_path, html_path.parent), quote=True)
...
<meta name="generator" content="fsqc_report.py README-to-HTML converter">
<title>{html.escape(html.unescape(title))}</title>
<style>{MARKDOWN_REPORT_CSS}</style>
</head>
<body>
<nav class="source-bar"><a href="{readme_link}">View canonical README.md</a></nav>
<main class="markdown-body" data-report-source="README.md">
{body}
</main>
<footer class="footer">Converted from <code>README.md</code> by <code>fsqc_report.py</code> · Research use only</footer>
</body>
</html>
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Generate/update the canonical README.md, then convert it to a styled "
            "fsqc-report.html beside the input CSV."
        ),
    )
    parser.add_argument(
        "--regions-file",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.readme or args.html:
        ...
        try:
            ...
            return 1
        role = "canonical pictorial README" if args.html else "pictorial README"
    ...
    if args.html:
        ...
        try:
            ...
            html_report = format_html_from_markdown(
